...
namespace PGS
{
    internal class Functions
    {
        ...
        public static void BuildAndAssessArguments(string[] Arguments)
        {
            // Check to see if there are additional arguments.
            if (Arguments.Length > 1)
            {
                // The arguments position where game arguments actually begin.
                int GameArgsPos = 1;

                // The second argument can contain special flags to do cool stuff.
                switch (Arguments[1])
                {
                    // Check to see if the second argument is to disable the timer.
                    case "-notimer":
                    {
                        // Timer has been disabled so store that.
                        Globals.NoTimer = true;

                        // Increment the game arguments since this slot was used.
                        GameArgsPos++;

                        // Mandatory break command that doesn't merit a comment.
                        break;
                    }
                    // Check to see if the second argument is to skip the GUI.
                    case "-nogui":
                    {
                        // The user doesn't want a GUI so don't display it.
                        Globals.NoGUI = true;
                ...
                for (int i = GameArgsPos; i < Arguments.Length; i++)
                {
        ...
        public static void SetPrimaryAndLaunch()
        {
            ...
            if (!Globals.NoGUI) { Forms.WaitDialog.Show(); }
